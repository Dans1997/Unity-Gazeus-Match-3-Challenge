# Nightseeker: Gazeus Games Developer Assessment

![Nightseeker Logo](logo.png)
...
* **READme**: Updated Readme to include some of the limitations of my code and my general decision-making approach.

*(Future days will detail added features, fixes, and design choices.)*

--- 
##  Very-Likely Future FAQ
> Some things I jotted down while making this prototype that are very likely to be asked about my project.

### Q: Why didn’t you migrate the UI to UI Toolkit?
**A:** One week scope. I prioritized core systems (modular board engine, DI-style initialization, Addressables, pooling, deterministic mechanics). 
Views are already instantiated as addressable prefabs and controlled via view interfaces, which makes a later UI Toolkit migration more time-consuming than difficult.

**Next steps:**
* Create UI Toolkit view prefabs and adapters for existing view interfaces (`IMenuView`, `IGameplayView`).
* Swap addressable view keys in `GameConfig` and add a small migration PR.

---

### Q: How stable is the board logic?
**A:** The board engine was modularized (tile generator, match finder, swap service), improving maintainability, but refactors much probably introduced edge-case bugs (match-finder corner cases, anchor assumptions for effects, etc). 
The architecture is solid — stability requires targeted fixes and tests, but that takes a lot of time and planning.
I consider what I did modular but inneficient, though I'm satisfied because I didn't really have a lot of time to think about how to implement this.

**Next steps:**
* Unify `TileInfo` semantics. There are too many data models that basically work for the same purpose (`AddedTileInfo`, `MovedTileInfo`, `BoardTileInfo`).
* Add unit tests for every service to make sure board logic is solid. If I had more time, I'd surely change the basic data structure for the board logic. IList<IList<>> is just not good enough and introduces quadratic algorithms.
* Full board copies were maintained because of time constraints, but they'd be one of the first things I'd try to remove were this a serious project.

---

### Q: Is asset lifecycle and unloading safe (Addressables, audio, pools)?
**A:** I think it's good enough for the scope of the prototype but weak for a full game. Match 3 games are full of different effects and customizables, so more time would have to be put in the asset loading and unloading.

**Next steps:**
* Audit every `Load`/`Instantiate` and ensure matching `Release`/`ReleaseInstance` or `LeanPool.Despawn`.
* Add an integration test that loads/unloads scenes repeatedly and asserts stable memory/asset counts.
* A remote CDN service is always a good choice because of publishing hassle, so my first instinct would be to go with that.

---

### Q: Why is the scoring system "unfinished?"
**A:** Time (the answer is always time). Although "unfinished" is relative (the scoring works, it just doesn't have multipliers, combos and the like), I am indeed not satisfied with how it is now.
It's boring. The original idea was to have it hooked to board effects, matching cascades, etc. But I prioritized architecture (which makes adding more score mechanics easy).

**Next steps:**
* Implement `IScoreRule` variants (base, length bonus, cascade multiplier, special-piece bonus).
* Hook `ScoreService` to the `BoardEvent` stream.

---

### Q: Does the match finder produce rich metadata needed for advanced mechanics (L/T shapes, correct anchor)?
**A:** Not fully yet. The pipeline moved from a boolean matrix to an `ITileMatchRule` system, but match-finder output still needs richer `MatchGroup` objects (positions, shape, anchor, tile key) to reliably create special pieces and accurate effects.

**Next steps:**
* Change match finder output to `MatchGroup { List<Vector2Int> Positions; TileKey Key; MatchShape Shape; Vector2Int Anchor; }`.
* Use `Anchor` for spawning power-ups and computing area-of-effect logic.
* Right now, you can't really understand what is going on unless you look at the `GameConfig`. This is one of the biggest flaws in my system. 
---

### Q: Board generation is complex with many rules — how maintainable and fast is it?
**A:** The generation checks rules to avoid starting matches and that gets more expensive when many rules exist. The approach is mostly correct but should be optimized.

**Next steps:**
* Use a seeded `IRandom` plus a limited retry/backtrack algorithm (retry N times per cell; backtrack on dead-ends).
* Maybe expose retry limits and performance knobs in `BoardConfig`.
* Maybe add generation benchmarks.

---

### Q: Where are the tests?

**A:** Tests were not added due to time constraints, but the refactor isolates pure C# services so unit tests are straightforward to add.

**Next steps:**
* Add a `Tests` assembly and deterministic tests using seeded `IRandom`.

---

## What Was Asked vs What Was Delivered
> Quick, explicit mapping so evaluators can rapidly see requirement → result.

| Asked                                      | Delivered                                                                                                                                                                                                                                                   |
|--------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Scoring system                             | Scoring is boring but works as intended: a simple sum of tiles matched plus cascading bonus. The architecture supports an easy extension.                                                                                                                   |
| Advanced mechanics (line clear, explosions, same-color wipe) | All three were implemented. Right now, they are respectively triggered by 4-Tile Linear Matches, 4-Tiled Fire Tile Destructions, and 4-Tiled Demon Tile Destructions. This can be configured in `GameConfig`.                                               |
| Animations & visual feedback               | DOTween-based animations, LeanPool pooling, placeholder tile explosion, selection feedback and hint system. Look all over the place in my opinion but could be easily improved (of course with an artist that knows what they're doing instead of yours truly). |
| Progression & rules                        | Three `GameEndRules` and four `TileMatchRules` were added. They can be added and customized in `GameConfig`.                                                                                                                                                |
| Gameplay variations & special pieces       | The only thing I can think of right now that I didn't deliver was a type of tile that could be matched with any other and produce a certain special effect on the board. However, you can see in `GameConfig` that any match could easily become rarer.     |

**My Verdict:** Core architecture and features are delivered: modular board engine, DI-like initialization, addressables, pooling, and board effects. Remaining work is primarily stability, tests, tuning, and generating/providing/displaying metadata.
...
1. Open the project in the Unity editor matching the project version used to build this (project was developed on Unity 2022).
2. Build Addressables (Addressables → Build Player Content) if necessary.
3. Open the **Initialization** scene and press Play. The bootstrapper will:

    * Initialize Addressables,
    * Load `GameConfig`,
    * Instantiate the Main Menu or Loading Screen,
    * Let you navigate to Gameplay via Main Menu → Play.

### How To Configure The Game (Editor)
> Note: I'm not sure how this will run without `OdinInspector`. It's been a long time I haven't done a project without it. Unity's serialization is solid but frankly way too simple for full-fledged games.

Right now, there isn't a way to configure the game during runtime, though it would only need an `OptionsController` and a `OptionsControllerView` that would work to return a `GameConfig`. 
Fortunately, anything you want to change is injected:
* **Single source of truth**: `GameConfig` (inside Assets folder) a ScriptableObject that injects gameplay, effects, and end-game rules into the systems.
* **Change it, change the game**: edits here alter behavior without touching code.

#### Gameplay Info

* **Board Height / Width**: set grid size.
* **Gameplay View Prefab Key**: choose the Addressable key for the in-game UI/prefab.
* **Game Over View Prefab Key**: choose the Addressable key for the game-over view.
* **Board Cell View Prefab Key**: key for the visual cell prefab (per board square).
* **Board Tile View Prefab Key**: key for the visual tile prefab (per tile).

#### Available Tile Configs

* **Holds all tile types** used by the game (normal + specials).
* **Add a tile**: click **+**, reference/create a `TileConfig` asset, and set its visuals/behaviors there.
* **Remove a tile**: click **x**; don’t leave orphaned references in rules/effects.

#### Tile Match Rules

* **Purpose**: define what counts as a “match” the resolver should detect.
* **Each rule = (Match Type, Min Length)**.
* **Add a match rule**: **+** → pick **Tile Match Type** (e.g., *Horizontal Line Match*) → set **Min Length** (e.g., `3`).
* **Edit an existing rule**: change type or min length inline.
* **Disable a pattern**: remove its rule (resolver won’t look for it).

#### Board Effect Configs (when something happens after matches/destructions)

> Each item = one effect with optional triggers.

* **Board Effect Type**: pick what to do:
    * *Horizontal Clear Effect*: clears the whole row of the triggering tile.
    * *Vertical Clear Effect*: clears the whole column.
    * *Clear Same Tile Type Effect*: clears all tiles of the same type as the trigger.
    * *Square Explosion Board Effect*: clears a square around the trigger; uses **Explosion Radius**.
* **Tile Match Rule Triggers (optional)**: restrict this effect to fire **only** on specific match patterns.
    * Example: add *Horizontal Line Match* to make an effect fire only after horizontal matches.
    * **Leave empty** to allow any match type to trigger (if other triggers are met).
* **Destroyed Tile Triggers (optional)**: restrict by **which tiles were destroyed** and **how many**.
    * **What it is**: a **HashSet** of tile keys (e.g., *Demon Tile*, *Fire Tile*).
    * **Min Value**: minimum count of those tiles destroyed in a single resolution to fire the effect (e.g., `4`).
    * Example: `Demon Tile` + **Min Value 4** → effect fires only if ≥4 Demon tiles were destroyed.
* **Explosion Radius** (only for explosion effects): size of the square around the trigger (`1` = 3×3, `2` = 5×5, etc.).

#### Game End Rules

* **You can stack multiple end conditions**; the first one satisfied ends the run.
* **No Moves Left Rule**: ends when no valid swaps remain.
* **Score Threshold Rule**: ends when **Score ≥ Score Threshold** (e.g., `1000`).
* **Timer Rule**: ends when **Timer Seconds** elapse; **Update Interval In Seconds** is how often the timer ticks/updates.

#### Quick recipes (copyable steps)

* **Add a new match pattern**: *Tile Match Rules* → **+** → choose pattern (e.g., *Vertical Line Match*) → set **Min Length**.
* **Make rows clear on long horizontals**: *Board Effect Configs* → **+** → **Horizontal Clear Effect** → add **Tile Match Rule Trigger = Horizontal Line Match** → set your match rule’s **Min Length** to the desired threshold.
* **Color clear on Demon Bursts**: **Clear Same Tile Type Effect** → **Destroyed Tile Triggers = { Demon Tile }** → **Min Value = 4**.
* **Bomb on Fire Bursts**: **Square Explosion Board Effect** → **Explosion Radius = 1** → **Destroyed Tile Triggers = { Fire Tile }** → **Min Value = 4**.
* **Column clear on verticals only**: **Vertical Clear Effect** → **Tile Match Rule Triggers = { Vertical Line Match }**.
* **8×8, win by score, no timer**: *Gameplay Info* → **Width/Height = 8** → *Game End Rules* keep **Score Threshold Rule (e.g., 1500)**, remove **Timer Rule**.
* **Timed mode, 5 minutes**: *Game End Rules* → ensure **Timer Rule** exists → **Timer Seconds = 300**, **Update Interval = 1**.
* **Disable “no moves left” loss**: remove **No Moves Left Rule** (use with care—softlocks possible).

#### Before You Play

* **Tile types referenced in triggers exist** in *Available Tile Configs*.
* **Prefab Keys resolve** to valid Addressables.
* **At least one match rule** is defined (or nothing will ever match).
* **At least one end rule** exists (or sessions never end).

## Final Notes

* The board-centric architecture (modular services) was intentionally prioritized because it scales: once `MatchGroup` metadata, deterministic generation, and event streams are stable, adding new features (special pieces, rules, scoring) becomes low-risk and testable.
* Addressables + prefab injection + pooled views keep runtime memory/use manageable and allow quick changes to visuals without recompiling systems.
* The whole point of this was to show that I'm more preocuppied in making an efficient development environment (quality) than in quickly delivering good-looking assets that would have to be trashed if it were to become a real game. I'm confident that this could easily become the codebase for a real, published Match 3 game.
* That said, by no means I think this is necessarily better than other dev approaches. My code can be jarring to people used to Unity's scene-based, component-based workflow. And I get it. It's weird, though I believe it's much better than creating 30 singletons. I hope that the evaluators will see this not as an attempt to say my code is better, but that I value organization, maintainability, and a codebase that will help us solve our problems, and in my experience this has to be compensated by a more flexible, generic code at first.